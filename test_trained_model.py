...
# Plot prediction
plot_results_directory = os.path.dirname(os.path.abspath(__file__)) + "\\Results"
plt.plot(test_target, label="Actual Data")
for i in range(len(model_predictions)):
    plt.plot(model_predictions[i], label=f"{chosen_models[i]} Predictions")
plt.title('All models predictions')
plt.ylabel(f"{feature_list[selected_feature%5]}")
plt.xlabel("Time")
plt.legend()
figure_path = os.path.join(plot_results_directory, "all_models_predictions.png")
...
figure_path = os.path.join(plot_results_directory, "all_models_predictions_context.png")
